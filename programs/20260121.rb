...
def draw
  ...
  bottom_band_height = 70
  grid_area_height = 700 - bottom_band_height

  # 不均一なグリッドの行・列幅を生成
  cols = generate_divisions(700, 8, 12)
  rows = generate_divisions(grid_area_height, 8, 12)
  
  center_x = 350
  center_y = grid_area_height / 2.0
  ...
  cols.each do |col_width|
    ...
    rows.each do |row_height|
      ...
      dist = sqrt((cell_center_x - center_x)**2 + (cell_center_y - center_y)**2)
  ...
  fill(0, 0, 10)
  rect(0, grid_area_height, 700, bottom_band_height)

  fill(0, 0, 95)
  textAlign(LEFT, CENTER)
  textFont("Arial Black")
  textSize(32)
  self.text("GENUARY 2026 #21", 30, grid_area_height + bottom_band_height / 2)
end
