...
def draw
  ...
  cols.each do |col_width|
    ...
    rows.each do |row_height|
      ...
      # セル内グラデーション
      draw_cell_gradient(x, y, col_width, row_height, base_hue, base_saturation, base_brightness)
...
def draw_cell_gradient(x, y, w, h, base_hue, base_sat, base_bri)
  steps = 8
  ...
  steps.downto(1) do |i|
    ...
    # 外側は暗く、内側は明るく
    brightness_offset = (1 - t) * 12 - 6  # -6 〜 +6
    saturation_offset = (1 - t) * -8 + 4  # +4 〜 -4（中心は彩度やや低）
