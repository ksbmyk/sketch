...
FRAGMENT_SHADER = <<~GLSL
  ...
  void main() {
    ...
    // Vivid but soft colors
    float hue = 0.55 + n * 0.1 + sin(u_time * 0.15) * 0.03;
    float sat = 0.5 + n * 0.2; // Higher saturation
    float val = 0.6 + n * 0.3;

    vec3 col = hsv2rgb(vec3(hue, sat, val));

    // Add cyan/blue tint to lighter areas
    float light = smoothstep(0.4, 0.7, n);
    col += vec3(0.1, 0.25, 0.35) * light;

    // Gentle highlights
    float highlight = smoothstep(0.65, 0.85, n);
    col += vec3(0.15, 0.3, 0.4) * highlight;

    // Floating light particles
    float pts = particles(uv, u_time);
    vec3 particleColor = vec3(0.7, 0.9, 1.0);
    col += particleColor * pts * 0.5;

    // Soft particle glow
    float ptsGlow = particles(uv * 0.95, u_time);
    ptsGlow += particles(uv * 0.9, u_time) * 0.5;
    col += vec3(0.4, 0.7, 0.9) * ptsGlow * 0.1;
